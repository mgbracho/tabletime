...
export function GroceryListView({
  ...
  const shareList = async () => {
    ...
    try {
      await navigator.share({ title: "Lista de la compra - TableTime", text: getExportText() });
    } catch (err) {
  ...
  const handlePrint = () => {
    ...
    const itemsHtml = allItems
      ...
      .join("");
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Lista de la compra</title><style>body{font-family:system-ui,sans-serif;padding:24px;max-width:400px;margin:0 auto;font-size:14px}h1{font-size:1.25rem;margin:0 0 8px}p{color:#555;margin:0 0 16px}ul{list-style:none;padding:0;margin:0}</style></head><body><h1>Lista de la compra - TableTime</h1><p>Semana del ${escape(weekTitle)}</p><ul>${itemsHtml}</ul></body></html>`;
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-teal-200 bg-amber-50/70 px-4 py-3">
        ...
        {allItems.length > 0 && (
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={downloadTxt} className="rounded-lg border border-teal-200 px-3 py-1.5 text-sm font-medium text-teal-700 hover:bg-teal-50">Descargar .txt</button>
            {"share" in (typeof navigator !== "undefined" ? navigator : {}) && (
              <button type="button" onClick={shareList} className="rounded-lg border border-teal-200 px-3 py-1.5 text-sm font-medium text-teal-700 hover:bg-teal-50">Compartir (Notas/Recordatorios)</button>
            )}
            <button type="button" onClick={handlePrint} className="rounded-lg border border-teal-200 px-3 py-1.5 text-sm font-medium text-teal-700 hover:bg-teal-50">Imprimir</button>
          </div>
        )}
      </div>

      <form onSubmit={addManual} className="flex gap-2">
        <input type="text" value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder="Añadir producto (ej. Leche, Pan)" className="flex-1 rounded-lg border border-teal-200 px-3 py-2 text-sm focus:border-teal-400 focus:outline-none focus:ring-1 focus:ring-teal-400" />
        <button type="submit" className="rounded-lg bg-teal-600 px-4 py-2 text-sm font-medium text-white hover:bg-teal-700">Añadir</button>
      </form>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <li key={item.id} className={`flex items-center gap-3 rounded-lg border px-3 py-2 ${checked ? "border-teal-100 bg-teal-50/50 text-zinc-500 line-through" : "border-teal-100 bg-white"}`}>
                      <button type="button" onClick={() => toggle(item.id)} className="flex h-5 w-5 shrink-0 items-center justify-center rounded border border-teal-300 text-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-400" aria-label={checked ? "Marcar como no comprado" : "Marcar como comprado"}>
                        {checked ? "✓" : ""}
                      </button>
                      <span className="min-w-0 flex-1 text-sm">{item.label}</span>
                      {!item.fromPlan && (
                        <button type="button" onClick={() => removeManual(item.id)} className="shrink-0 rounded p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600" aria-label="Quitar">✕</button>
                      )}
                    </li>
                  );
                })}
